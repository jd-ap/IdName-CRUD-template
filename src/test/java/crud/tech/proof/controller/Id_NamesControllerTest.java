...
class Id_NamesControllerTest {
    ...
    void givenFindAll_whenTest1_thenReturns200() {
        //given
        String url = "/id-names?pageNumber=%d&pageSize=%d&sort=%s";
        ...
        //when
        var result = restTemplate.getForEntity(url.formatted(pageNumber, pageSize, sort), IdNameDto[].class);
        ...
        assertAll("Verify headers",
                () -> assertEquals("0", headers.getFirst("x-pagination-current")),
                () -> assertEquals("true", headers.getFirst("x-pagination-hasNext")),
    ...
    void givenFindByName_whenTest3_thenReturns200() {
        //given
        String url = "/id-names/name/%s?pageNumber=%d&pageSize=%d&sort=%s";
    ...
    void givenUpdateById_whenTest5_thenReturns200() {
        ...
        command.setName("fixed.name");

        var expected = new IdNameDto().id(aId).name("fixed.name").active(Boolean.TRUE);
    ...
    void givenCreateNew_whenTest6_thenReturns201() {
        ...
        assertEquals("created name", result.getBody().getName());
